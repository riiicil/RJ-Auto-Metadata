...
>
> **v3.12.3** (June 2026) is a feature and bugfix release integrating the Aivene OpenAI-compatible provider and resolving the keyword space-stripping bug in the metadata generation pipeline.
>
...
## Phase 4F Status: Complete (Aivene & Keyword Spaces)

### What Was Done

- **Aivene Provider Integration**: Integrated support for Aivene as a new OpenAI-compatible AI gateway provider. Created `src/api/aivene_api.py` and registered it in `provider_manager.py`. The provider is loaded dynamically into the UI dropdown.
